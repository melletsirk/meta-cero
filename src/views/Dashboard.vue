    ...
    <div class="flex items-start gap-4 p-4 sm:p-5 bg-amber-50 border border-amber-300 rounded-2xl shadow-sm stagger-3">
      ...
      <div>
        ...
        <p class="text-sm text-amber-800 leading-relaxed">
          ...
          a
          <span class="inline-flex items-center gap-1 font-semibold"><span class="w-2 h-2 rounded-full bg-amber-500 inline-block"></span>CPP</span>.
    ...
    <div class="glass rounded-3xl shadow-sm border border-white/60 p-6 sm:p-8 stagger-3">
      ...
      <div v-else class="space-y-4">
        ...
        <div v-for="(deuda, index) in deudasStore.deudas" :key="deuda.id"
          ...
          <div class="mt-4 sm:mt-0 text-left sm:text-right w-full sm:w-auto flex flex-col sm:items-end justify-between border-t border-slate-100 sm:border-0 pt-3 sm:pt-0 gap-3 sm:gap-2">
            <div class="flex sm:flex-col justify-between items-center sm:items-end w-full">
              <p class="font-extrabold text-lg text-slate-900">{{ formatMonto(deuda.monto_pendiente, deuda.moneda) }}</p>
              <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider mt-0 sm:mt-1" :class="{
